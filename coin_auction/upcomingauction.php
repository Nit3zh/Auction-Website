...
if (isset($_GET['acid'])) {
    $_SESSION['acid'] = $_GET['acid'];
    header("location: auctionsh.php");
    exit;
}
            ...
            if($result->num_rows>0){
                while ($rec = $result->fetch_assoc()){
                    echo "<div class=`log` style=\"border:1px solid black; margin:10px;\">
                            ...
                            <form method=`get`>
                                <button type='submit' name='acid' value=".$rec["acid"]." style='margin-right: -426px; margin-bottom: 10px;'>Start Bidding</button>
                            </form>
                          </div>";
                }
            }
